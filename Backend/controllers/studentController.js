...
const getJobPlacementStudents = (req, res) => {
  const query = `
    SELECT s.FirstName, s.LastName, s.PhoneNo, s.Email, s.Program, p.year,p.company_name, p.Status, p.package, p.position, p.Notes FROM students s JOIN placement_details p ON s.Id = p.student_id WHERE s.Career_Choice = 'Job Placement';
  `;
...
const getHigherStudiesStudents = (req, res) => {
  const query = `
    SELECT s.FirstName, s.LastName, s.Email, s.Enrollment_Year, s.Program, h.university_name, h.course_name, h.intake_year, h.Status
    FROM students s
