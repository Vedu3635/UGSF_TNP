...
const deleteStudent = async (req, res) => {
  ...
  try {
    ...
    const placementDeleteSql = 'DELETE FROM placement_details WHERE student_id = ?';
    const placementResult = await pool.query(placementDeleteSql, [studentId]);

    // Delete from higher studies details
    const higherStudiesDeleteSql = 'DELETE FROM higher_studies_details WHERE student_id = ?';
    const higherStudiesResult = await pool.query(higherStudiesDeleteSql, [studentId]);
